...
//                e.let { event ->
//                    val child = courseRecyclerView.findChildViewUnder(event.x, event.y)
//                    if (child != null) {
//                        val position = courseRecyclerView.getChildAdapterPosition(child)
//                        val courseName = allCourses[position].title
//                        val action = CourseCategoryFragmentDirections
//                            .actionCourseCategoryFragmentToProccedCourseFragment(courseName)
//                        findNavController().navigate(action)
//                    }
...
//            courseRecyclerView.adapter = courseAdapter
//
//            // Set the touch listener on the RecyclerView to handle gestures
////            courseRecyclerView.setOnTouchListener { _, event ->
////                gestureDetector.onTouchEvent(event)
////                false // Return false to allow RecyclerView to handle other touch events
////            }
//
//            showMoreCourseButton.visibility = if (courses.size > 6) View.VISIBLE else View.GONE
//        }
...
class CourseCategoryFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_course_category, container, false)
        val backButton: LinearLayout = view.findViewById(R.id.fixedInclude)
